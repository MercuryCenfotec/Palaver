...
@Component({
    selector: 'jhi-ban-update',
    templateUrl: './ban-update.component.html'
})
export class BanUpdateComponent implements OnInit {
    ban: IBan;
    isSaving: boolean;
    ...
    save() {
        ...
        if (this.ban.id !== undefined) {
            this.subscribeToSaveResponse(this.banService.update(this.ban));
        } else {
            this.subscribeToSaveResponse(this.banService.create(this.ban));
        }
    }

    protected subscribeToSaveResponse(result: Observable<HttpResponse<IBan>>) {
        result.subscribe((res: HttpResponse<IBan>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
