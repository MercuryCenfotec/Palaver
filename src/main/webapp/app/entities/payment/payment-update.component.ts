...
export class PaymentUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.balanceAccountService
            .query()
            .pipe(
                filter((mayBeOk: HttpResponse<IBalanceAccount[]>) => mayBeOk.ok),
                map((response: HttpResponse<IBalanceAccount[]>) => response.body)
            )
            .subscribe((res: IBalanceAccount[]) => (this.balanceaccounts = res), (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackBalanceAccountById(index: number, item: IBalanceAccount) {
        return item.id;
    }
}
